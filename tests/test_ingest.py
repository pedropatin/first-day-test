...
def test_rejections_keep_lineage(raw_dir, db_path):
    ...
    rows = con.execute(
        ...
        "from bronze.raw_ingest_rejections order by line_number"
    ).fetchall()
    assert len(rows) == 2
...
def test_reingest_is_idempotent(raw_dir, db_path):
    ...
    run_ingest(raw_dir, db_path)
    con = duckdb.connect(str(db_path), read_only=True)
    assert con.execute("select count(*) from bronze.raw_events").fetchone()[0] == 18
    assert con.execute("select count(*) from bronze.raw_ingest_rejections").fetchone()[0] == 2
    assert con.execute("select count(*) from bronze.raw_accounts").fetchone()[0] == 2
...
def test_raw_preserves_original_payload(raw_dir, db_path):
    ...
    ts, payload = con.execute(
        "select event_ts, raw_payload from bronze.raw_events where event_id = 'e16'"
    ).fetchone()
